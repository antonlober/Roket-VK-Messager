...
public class TextViewSpan {
    ...
    public static void parse(final TextView view) {
        String text = view.getText().toString();
        if (TextUtils.isEmpty(text)) {
            return;
        }
        // text = Emoji.replaceEmoji(text).toString();
        Spannable spannable = new SpannableString(text);

        Pattern pattern = URL_PATTERN;
        Matcher matcher = pattern.matcher(text);
        while (matcher.find()) {
            int span = Color.parseColor("#6B9CC2");
            if (ApplicationName.colors != null) {
                span = ApplicationName.colors.textColor;
            }
            spannable.setSpan(new ForegroundColorSpan(span), matcher.start(1), matcher.end(1),
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            spannable.setSpan(new UnderlineSpan(), matcher.start(1), matcher.end(1),
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
