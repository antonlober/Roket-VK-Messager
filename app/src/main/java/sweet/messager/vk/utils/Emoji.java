...
public class Emoji {
    ...
    private static HashMap<Long, String> rects = new HashMap<>();
    ...
    static {
        for (int j = 1; j < data.length; j++) {
            for (int i = 0; i < data[j].length; i++) {
                ...
                rects.put(data[j][i], "test");
            }
        }
    }

    public static String replaceEmoji(CharSequence cs) {
        StringBuilder _return = new StringBuilder(cs);
        long buf = 0;
        int emojiCount = 0;
        try {
            for (int i = 0; i < cs.length(); i++) {
                ...
                if (c == 0xD83C || c == 0xD83D || (buf != 0 && (buf & 0xFFFFFFFF00000000L) == 0 && (c >= 0xDDE6 && c <= 0xDDFA))) {
                    ...
                    buf <<= 16;
                    buf |= c;
                    String d = Emoji.getEmojiDrawable(buf);
                    if (d != null) {

                        emojiCount++;
                        if (c >= 0xDDE6 && c <= 0xDDFA) {
                            _return.replace(i - 3, i + 1, "|emoji|");
                        } else {
                            _return.replace(i - 1, i + 1, "|emoji|");
                        }
                    }
                    buf = 0;
                } else if (c == 0x20E3) {
                    if (i > 0) {
                        ...
                        if ((c2 >= '0' && c2 <= '9') || c2 == '#') {
                            ...
                            String d = Emoji.getEmojiDrawable(buf);
                            if (d != null) {
                                Log.e("emojiCode", getEmojiCode(c));
                                emojiCount++;
                                _return.replace(i - 1, i + 1, "|emoji|");
                            }
                            buf = 0;
                    ...
                    String d = Emoji.getEmojiDrawable(c);
                    if (d != null) {
                        Log.e("emojiCode", getEmojiCode(c));
                        emojiCount++;
                        _return.replace(i, i + 1, "|emoji|");
                    }
                }
                if (emojiCount >= 50) {
                    break;
    ...
    public static String getEmojiDrawable(long code) {
        String info = rects.get(code);
        if (info == null) {
    ...
    public static String getEmojiCode(char c) {
        String utf8tweet = "";
        try {
            // byte[] bytes = String.valueOf(c).getBytes();
            utf8tweet = String.valueOf(c);
            /*
